  ...
  try {
    ...
    for (const term of searchTerms) {
      ...
      try {
        ...
        if (isStablecoinProject && price < 0.01) continue
...
async function fetchDefiLlama(projectName: string, handle: string) {
  try {
    ...
    const slugsToTry = [
      slug, handleSlug, slug.replace(/-/g, ''), handleSlug + '-protocol',
      handleSlug + '-fi', words.slice(0,3).join('-'), words.slice(0,2).join('-'),
      acronym, handle.toLowerCase(),
    ...
    return {
      tvl: tvlStr, fees_24h: feesStr, revenue_24h: revenueStr, total_raised: raisedStr,
      investors: investors.slice(0, 5), category: data.category || null,
      chains: data.chains?.slice(0, 3) || [], audits: data.audits || [],
    }
  } catch { return null }
}
...
async function fetchRootData(projectName: string, apiKey?: string) {
  try {
    ...
    const team = (d.team_members || []).map((t: any) => ({
      name: t.name || '', role: t.title || '',
      x_handle: t.twitter ? '@' + t.twitter.replace('https://twitter.com/', '').replace('@', '') : '',
    })).filter((t: any) => t.name).slice(0, 5)

    return { total_raised: totalRaised, investors, team, tags: d.tags || [], one_liner: d.one_liner || '', description: d.description || '' }
  } catch { return null }
}
...
async function fetchCryptoNewsSentiment(projectName: string, ticker?: string) {
  try {
    ...
    const projectTerms = [projectName.toLowerCase(), (ticker || '').toLowerCase()].filter(Boolean)
    const relevantArticles = articles.filter((a: any) => {
      const titleLower = (a.title || '').toLowerCase()
      const descLower = (a.description || a.body || '').toLowerCase()
      return projectTerms.some(term => term.length > 2 && (titleLower.includes(term) || descLower.includes(term)))
    })
    const articlesToUse = relevantArticles.length >= 2 ? relevantArticles : articles
    const titles = articlesToUse.map((a: any) => a.title || '').filter(Boolean)
...
function detectFUDSignals(u: any, intel: any, dexData: any, hacksData: any[], newsData: any) {
  ...
  if (hasRealXData && ageMonths < 6 && followers > 10000) {
    flags.push({ type: 'suspicious', label: 'New account with high followers', detail: `Account is only ${Math.round(ageMonths)} months old but has ${followers.toLocaleString()} followers — possible purchased followers or rebranded account.`, severity: 'medium' })
  }

  if (hasRealXData && following > followers * 2 && followers < 5000) {
    flags.push({ type: 'suspicious', label: 'Follow-farming detected', detail: `Following ${following.toLocaleString()} accounts but only ${followers.toLocaleString()} followers — classic follow-for-follow farming pattern.`, severity: 'low' })
  }

  if (hasRealXData && followers > 50000 && listed < followers * 0.001) {
    flags.push({ type: 'suspicious', label: 'Low credibility ratio', detail: `Only ${listed} accounts have listed this project despite ${followers.toLocaleString()} followers — suggests low genuine engagement or bot followers.`, severity: 'low' })
  }

  if (hasRealXData && ageMonths > 12 && tweetCount < 50) {
    flags.push({ type: 'other', label: 'Inactive account', detail: `Account is ${Math.round(ageMonths)} months old but only has ${tweetCount} tweets — possibly abandoned or not genuinely active.`, severity: 'low' })
  }

  if (dexData?.dump_detected) {
    const change = dexData.price_change_24h || 0
    flags.push({ type: 'dump', label: 'Token dump detected', detail: `Token price dropped ${Math.abs(change).toFixed(1)}% in 24h — significant sell pressure detected.`, severity: 'high' })
  }
  ...
  if (hacksData && hacksData.length > 0) {
    hacksData.forEach((hack: any) => {
      flags.push({ type: 'exploit', label: 'Security exploit on record', detail: `${hack.classification || 'Hack'} detected — ${hack.amount} lost via ${hack.technique || 'unknown technique'}.`, severity: 'high' })
    })
  }
  ...
  if (hasRealXData && bigNameMentions.length >= 2 && intel.vcMentions.length === 0) {
    flags.push({ type: 'suspicious', label: 'Unverified name-dropping in bio', detail: `Bio mentions ${bigNameMentions.join(', ')} but no confirmed funding rounds found — possible false credibility claims.`, severity: 'medium' })
  }
...
async function fetchCoinPaprika(projectName: string, handle: string, ticker?: string | null) {
  try {
    ...
    if (!nameMatch && !ticker) return null

    return {
      coin_id: match.id, symbol: coin.symbol?.toUpperCase() || match.symbol?.toUpperCase(),
      name: coin.name || match.name, description: coin.description || '', team, contracts,
      price_data: price, token_live: !!price,
      tags: (coin.tags || []).map((t: any) => t.name || t).filter(Boolean).slice(0, 5),
      github: githubLink, twitter: twitterLink, started_at: coin.started_at || null,
      development_status: coin.development_status || null, proof_type: coin.proof_type || null,
    }
  } catch { return null }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // 2. Pinned tweet — SKIPPED to save X API credits ($0.005/scan)
    ...
    if (tokenData && !tokenData.market_cap_str && tokenData_cg?.market_cap_str) {
      ...
      tokenData = {
        token_live: true, ticker: cpData.symbol, token_price: cpData.price_data.price,
        market_cap_str: cpData.price_data.market_cap, volume_24h: cpData.price_data.volume_24h,
        price_change_24h: cpData.price_data.change_24h, contracts: cpData.contracts, source: 'coinpaprika',
      }
    }
